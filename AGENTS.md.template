...
- unfiltered Drupal readback in `drupal-readback.json`, produced by running the installed skill's `scripts/generate-readback.mjs` first. The generator owns the measured sections — `system.site` UUID, front-page setting, config sync directory, core version, enabled modules, themes, config status, node/media entity lists, per-bundle published/raw counts, and the per-measurement command/timestamp/output records — and those generator-written values must never be hand-authored or hand-edited; the live verifier recomputes them and fails on any mismatch. Builders then record the remaining inventory sections in the same file without altering generator-owned values: content types, field storage, form/view displays, Views, menus and menu links, all aliases including duplicates, Canvas pages when available, unexpected public routes, roles/permissions notes, and the owner/explanation fields of `publishedVersusRawDeltas` records the generator emits for raw-count deltas;
