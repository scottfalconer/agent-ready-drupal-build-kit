...
- Produce `review-packet/durable-intent.yml`. Leave `intent_records: []` only when live/readback evidence, the pattern map, and the off-road inventory show no content model, View, workflow, display, custom-module, or off-road architecture. Otherwise record intent or a named `empty_justification` with `rationale`, `accepted_by` distinct from the builder, `last_reviewed`, and packet-local `acceptance_evidence`. An empty intent file also vetoes contradictory maintainer claims that load-bearing decisions were captured.
