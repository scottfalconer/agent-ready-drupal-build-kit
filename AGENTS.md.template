...
- Produce `review-packet/durable-intent.yml`. Leave `intent_records: []` only when the pattern map declares no content types or Views and no off-road exceptions are recorded; otherwise the verifier requires intent records or an `empty_justification` block naming `rationale`, `asserted_by`, and `last_reviewed`. While the pattern map declares content types or Views, an empty intent file also vetoes the maintainer-review "load-bearing decisions captured" checkbox.
