...
The default source/target managed-browser floor also extracts visible native controls inside public forms on every primary route. It binds each control to privacy-safe one-based form and control ordinals, then compares normalized accessible identity plus occurrence when the source has one, native kind, visible label, required and disabled state, checkbox/radio selected and default state, and the ordered labels plus selected, default, and disabled state of every `<select>` option. An unlabeled source control may gain an accessible target identity at the same structural ordinal; the improvement is retained in the finding, while a missing, extra, reordered, or still-unlabeled target control fails. Hidden CSRF, honeypot, and other non-visible controls do not participate; internal field names, visitor-entered/current input values, and option values are never retained (an input submit/reset/button value may contribute only its rendered public label). Evidence is capped at 64 controls per route state, 64 options per control, and 180 characters per public label; an inaccessible target or any truncation fails closed. A select must expose a complete non-empty observed option state. If options load dynamically, make the populated state deterministic at the captured primary URL, using an exact query-state route where applicable; an empty source or target list is not a wildcard and authored prose cannot replace the verifier-owned DOM observation.
