...
- For every accepted collection, declare the source and target continuation modes as `paged`, `load_more`, `infinite_scroll`, or `none`, plus a positive observed page/batch size on each continuing side. Declare each continuation mechanism independently as an exact `request` or `browser_interaction`. Request-addressable states need exact distinct source and target requests; the verifier fetches them, confirms distinct final requests, and requires distinct visible-text or media semantics. JS-only states need exact non-default, interaction-bearing `publicRouteChecks.captureState.id` bindings and remain explicitly self-attested rather than verifier-replayed. Bind the target to one exact live View display unless an evidence-backed documented exception proves that the source is a genuine single-page collection. A continuing source cannot become `none`, and a `none` source cannot acquire target continuation. A source `load_more` or `infinite_scroll` experience may be replaced by `paged` or `load_more` only through an exact `acceptedBy` name, rationale, and packet-local evidence in an `accessible_continuation_replacement` disposition; source and target page sizes may differ. A `none` declaration requires null page sizes, `none` mechanisms, no continuation requests/state IDs, and reconciled rendered/entity counts proving that nothing was truncated.
...
- every accepted collection pagination declaration, including exact live View pager readback, positive observed source/target batch sizes for continuing collections, verifier-owned final-request plus semantic distinctness for exact request-addressable states, and explicitly self-attested interaction-bearing state evidence for genuinely browser-interaction-only continuation;
...
An intentional editorial difference can be accepted only with one `route-matrix.json.primaryNavigationParityDispositions` record using `schemaVersion: public-kit.navigation-parity-disposition.1`. Copy the exact desktop or mobile `expectedSourceTreeFingerprint`, `targetTreeFingerprint`, and sorted `differenceKinds` emitted by the verifier, then add `accepted: true`, a named `acceptedBy`, a rationale, and non-empty packet-local evidence. A broad “navigation reviewed” flag, a disposition for another viewport or tree fingerprint, duplicate dispositions, or missing evidence cannot waive the mismatch.
