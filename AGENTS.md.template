...
This command binds the packet to the identified live target and the current DDEV runtime by target origin, Drupal site UUID, front-page setting, config-sync directory, and clean config status. It independently requires real Git-tracked YAML in that current sync directory; fetches all primary and target-required routes; rejects non-success responses even when the packet reports the same `5xx`; inspects each fetched primary route's actual rendered canonical, meta description, and `og:image` against browser evidence; generates and verifies a missing route; checks access-wall canonicals and rendered internal legal/privacy links; and reconciles active Drupal consent config with verifier-owned before-consent network captures. Packet-authored before-consent browser evidence is not promoted to machine observation. It then writes `review-packet/evidence/live-verification.json`. It exits zero only when all required Drupal readback, authenticated editor/browser, independent-verification, and blind-review evidence authorizes local completion. Packet-only values and injected test runtimes cannot grant that authority. Exit `2` means the packet/live checks are valid but completion is still blocked; exit `1` means packet or live-target validation failed.
