...
- Reconcile every primary flexible route exactly once across the route matrix, `compositionModel.flexibleLandingRoutes`, `pageCompositionOwnership`, and independent composition-fidelity checks. Reconcile every declared editor-facing section exactly once with `sectionOwnershipMatrix`; a single global example is not enough.
