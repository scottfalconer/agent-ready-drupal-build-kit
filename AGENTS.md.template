...
- Inventory public source assets by stable row ID, asset class, visible source role, and discovered/reachable/unreachable counts. A raw asset count does not show whether a content image, download, icon, tracking pixel, or decorative asset was migrated into an appropriate role.
...
- For every required field in a recurring collection, declare and independently check at least one semantic integrity rule. Use population parity, normalized value parity, ordered-field comparisons, reference integrity, or non-placeholder checks as the source pattern requires. Equal entity counts do not pass when dates lose their ordering, references no longer resolve, values become placeholders, or populated source fields arrive empty.
...
- every declared recurring-content semantic rule, including target evaluation/pass/violation counts and Drupal readback for the same rule ID;
- reachable source-asset reconciliation, including managed Media, genuine external-provider ownership, item-level blockers, invalid source-to-target role assignments, and unexplained duplicate placements;
...
- field-semantic evidence: every required recurring-content field maps to a stable rule ID with source-applicable count, expected target pass count, Drupal readback, independent violation count, and item-level exception evidence.
- media-provenance evidence: source asset inventory rows reconcile to managed Media, genuine external-provider ownership, or item-level blockers, with no unresolved assets, invalid role assignments, or unexplained duplicate placements.
