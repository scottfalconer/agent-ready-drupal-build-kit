...
- Independent verification and a blind adversarial review that try to break the build's completion claims before handoff. A deterministic, state- and byte-bound bundle gives each reviewer only its strict input projection; both reviews reference the same root digest, and the final verifier re-discovers the declared input set and rejects target, state, projection, membership, or byte drift. The bundle remains builder-writable and does not authenticate a reviewer or issue a verdict. This catches "the checks pass, but this is not the requested site" failures.
- A `review-packet/` explaining architecture decisions, remaining gaps, human-only open decisions, and verification evidence. The default verifier prioritizes delivery feedback: it validates the packet and real DDEV target, primary and representative routes, browser/editor workflows, accessibility, consent, runtime state, tracked config, and review handoff before starting the separately budgeted source census; the crawl is deferred until those higher-priority checks pass. The late census checks the source homepage and declared primary routes before expanding through same-origin rendered links, robots sitemap directives, and bounded sitemap indexes/URL sets. It emits count-only progress, records status, final URL, title, H1, canonical, body hash, and discovery provenance, and defaults to 1,024 routes with coupled 2,048-request, 2,048-task, and 180-second bounds. Every reachable public source path must appear in the accepted route matrix; a builder-authored drift exclusion cannot hide it. Persistently machine-confirmed private or unreachable responses may use a structured boundary disposition without waiting for human review. If the late crawl exhausts a bound, the already-checked core target remains valid but final completion fails closed as machine-incomplete. After explicit owner authorization, one complete rerun may use `--source-max-routes` to expand every coupled bound up to 8,192 routes; partial crawls are never combined or accepted. The live target side is bounded the same way: route, link, redirect, and asset verification of the rebuilt site defaults to 1,000 accepted routes with a coupled 2,000-request budget, and a large rebuilt catalog exhausting that budget produces one structured `target.route-budget` blocker whose next action asks for owner authorization to rerun once with `--target-max-routes` (also up to 8,192 routes, scaling the coupled request, task, and deadline budgets). Packet-only data and injected test runtimes cannot authorize a completed rebuild.
...
- `bin/verify-assembly.mjs`: an optional launch-only exact-HEAD disposable verifier for provenance-bounded assembly planning, verifier-selected interrupted prefixes with owned database/file restoration, durable persistence and entity-identity checks, no-op reruns, and live-applicable extension survival; it never invokes Drupal in the working target or changes the default handoff verdict.
- `bin/verify-reproduction.mjs`: an optional host-side exact-HEAD disposable DDEV runner for digest-bound clean-install or snapshot maintainer/launch evidence; it never changes the default handoff verdict.
