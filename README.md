...
- `bin/verify-assembly.mjs`: an optional launch-only exact-HEAD disposable verifier for provenance-bounded assembly planning, first application, no-op reruns, live-applicable extension survival, and tested restoration after an observed partial failure; it never invokes Drupal in the working target or changes the default handoff verdict.
