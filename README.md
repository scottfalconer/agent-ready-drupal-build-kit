...
- The verifier also probes generated negative routes, access walls, rendered legal/privacy links, and active consent-controlled resources against the real DDEV target. Optional or disabled resources require verifier-owned per-route CDP capture under a fixed route ceiling and aggregate deadline, including selector-only or attachment-only controls that cannot be matched directly to a URL.
