...
- A `review-packet/` explaining architecture decisions, remaining gaps, human-only open decisions, and verification evidence. The default verifier also probes negative routes, access walls, rendered legal/privacy links, and active consent-controlled resources against the real DDEV target, using fresh per-route CDP network capture when optional or disabled resources must be absent before consent.
