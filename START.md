...
In source-site mode, that same full verifier performs a separately budgeted source census from `sourceBaseUrl` only after the higher-priority packet, target delivery, accessibility, runtime, and review-handoff checks pass. The census checks the homepage and declared primary routes before expanding through same-origin server-rendered links, `robots.txt` Sitemap directives, bounded sitemap indexes, and bounded URL sets. It emits count-only progress and stores source status, final URL, title, H1, canonical, body hashes, and provenance. Every newly discovered reachable public path must be represented by an accepted source route; a builder-authored legacy, test, or intentionally-drop disposition cannot clear it. If the default 1,024-route crawl exhausts a coupled bound, final completion remains blocked without relabeling the working core target as failed. After explicit owner authorization, rerun the whole verifier once with `--source-max-routes 2048` (or the exact next count reported by `agentContinuation`, up to 8,192); never merge partial crawls. Live verification of the rebuilt target's own routes and links is bounded the same way: a catalog beyond the default 1,000-route ceiling produces a structured `target.route-budget` blocker whose next action asks for owner authorization to rerun once with `--target-max-routes` (up to 8,192). A private or persistently unreachable response may use a matching machine-evidenced boundary record without waiting for human review. In brief mode, no source census runs; every accepted brief requirement must instead be mapped to target evidence and independently falsified.
