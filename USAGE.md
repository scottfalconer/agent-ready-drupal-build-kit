...
- unfiltered Drupal readback: run the kit's `scripts/generate-readback.mjs` first; it writes the measured sections (`system.site` UUID, front page, config sync directory, core version, enabled modules, themes, config status, node/media lists, and per-bundle published/raw counts) which must never be hand-written or hand-edited, then record the remaining inventory sections (content types, displays, Views, menus/menu links, all aliases including duplicates, Canvas pages when available, unexpected public routes, and roles/permissions notes) in the same file without altering generator-owned values;
