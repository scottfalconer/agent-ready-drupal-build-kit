...
Use the packet templates. Copy matching files from the kit's templates/ directory into review-packet/ as needed, remove the .template suffix in the target filename, and fill them with run-specific evidence or blocked stubs. Before final handoff, run:
...
- packet-verification.json from `node agent-ready-drupal-build-kit/bin/verify-packet.mjs --packet review-packet`, with a zero exit code before any complete local rebuild claim;
