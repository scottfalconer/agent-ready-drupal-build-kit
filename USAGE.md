...
- a collection ledger row for every declared list, grid, schedule, directory, archive, catalog, feed, gallery, or search-like route, with source/brief and target counts, Drupal ownership, non-admin editor add-a-row evidence, and an explicit query-backed versus input-and-editor-proven curated-reference selection; recurring/query-backed collections use Views, while curated references require a rationale, packet-local evidence, and exact single-page behavior; counts must match unless a recorded owner label, reason, and evidence disposition an exclusion (local attribution is self-attested), and a private or unreachable item needs evidence of that boundary;
...
- composition evidence showing the target's actual route owner matches the declared owner for machine completion; a target-bound deviation may name the fallback, rationale, accepter, and evidence for human review but cannot waive the mismatch;
...
In source-site mode, the verifier-owned source census runs only after the higher-priority packet, target delivery, accessibility, runtime, and review-handoff checks pass. Its primary phase fetches the homepage and declared primary routes before deep discovery follows same-origin rendered links and sitemap hints. Count-only status is written to stderr at phase boundaries, every 64 fast routes, and after a slower HTTP batch. The default census uses explicit 1,024-route, 2,048-request, 2,048-task, sitemap, body-size, concurrency, and 180-second bounds. Newly discovered reachable paths must be represented by accepted source routes.

Bound exhaustion does not turn a passing core target into `target.validation`; the report remains target-valid and exits `2` with a `source.census-budget` completion blocker. Its exact next action asks for owner authorization before one complete restart with `--source-max-routes 2048` or the next bounded count, scaling the coupled request, task, sitemap, and deadline budgets up to 8,192 routes. This option never accepts missing route-matrix rows and never resumes, merges, or blesses partial crawl results. At the maximum, record an owner-approved complete route-matrix treatment or ask a build-kit maintainer to revise the bound. Agent-resolvable reconciliation errors still require repair even when larger-crawl authorization is pending. This source-only work is `not_applicable` in brief mode and does not block the brief claim.

The live target side has the same shape. Verifying the rebuilt site's accepted routes, server-rendered links, redirects, and assets defaults to a 1,000-route ceiling with a coupled 2,000-request budget shared across those checks. A large rebuilt catalog that exhausts this budget produces one structured `target.route-budget` completion blocker instead of hundreds of per-route errors; pure budget-exhaustion findings are consolidated into that blocker while genuine route and link defects stay separately reported and agent-resolvable. The blocker's next action asks for owner authorization before one complete restart with `--target-max-routes 2000` or the next bounded count, scaling the coupled request, task, and deadline budgets up to 8,192 routes. It never accepts unrepresented links or missing route-matrix rows, and partial runs are never combined. This applies in both source-site and brief modes.
