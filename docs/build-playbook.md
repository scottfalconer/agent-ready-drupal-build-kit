...
The default verifier independently checks the source route inventory before relying on it. Under a separate source-only route/request/task/deadline budget, it fetches the source homepage and declared primary routes, follows same-origin rendered document links, reads robots sitemap directives and bounded sitemap indexes/URL sets, and stores only route metadata, provenance, and hashes. Every reachable public source path must be represented by an accepted source path in the route matrix. A builder cannot mark a public `2xx` route legacy, test, or intentionally dropped to clear completion; it remains implementation work. A `401`, `403`, `404`, or `410` boundary may be dispositioned without human review only after an immediate verifier recheck returns the same status and the route matrix contains the matching structured boundary. Truncation or budget exhaustion fails closed.
