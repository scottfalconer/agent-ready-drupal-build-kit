...
Every accepted non-redirect homepage and landing route must reconcile exactly once across the route matrix, `compositionModel.flexibleLandingRoutes`, `pageCompositionOwnership`, and independent composition-fidelity checks. Expensive desktop/mobile review remains risk-based, but a route cannot escape its composition and editor-owner decision merely because it is not a primary visual sample. Every declared editor-facing section must also reconcile exactly once with `sectionOwnershipMatrix`; a single global section example is not enough. Every Utility Page owner needs one exact source/target `utilityPageExceptions` row; aggregate labels such as “stable pages” are invalid. `buildTypeDeclaration` summarizes these route-level decisions and independently observed owners. It cannot enable or suppress Canvas evidence by itself, and its supported type, Canvas-availability evidence, source fit, editor implications, and acceptance must all be complete.
