...
The route matrix is exclusively the public/anonymous visitor surface. A denied, admin-only, authenticated-only, internal, or disabled route cannot satisfy public coverage. Record it instead in `drupal-readback.json routing.privateRoutes` with an exact route name/path and HTTP method, expected anonymous behavior, owner, rationale, and packet-local evidence; the live verifier checks those routes separately using the declared method.
...
When custom code is genuinely required, complete a separate structured solution ladder for each distinct capability in every custom module and theme. Give each ladder a stable `capabilityId`; give each testable acceptance criterion a stable ID; declare whether the capability is load-bearing; and require every partial core, Drupal CMS, Recipe, or contrib candidate to reference the exact unmet criterion IDs. Every custom route, discovered controller, machine-discovered theme ownership finding, and custom-behavior finding must reference a capability declared by its owning extension plus the exact source-surface IDs that implement it. A routing-YAML record binds to its own route-registration surface; a controller binds to its class/file surface; and a procedural theme finding binds to the nearest owning function surface. A controller without a route is still governed by its extension's capability review. Start with the exact need and criteria, then disposition Drupal core, installed Drupal CMS capabilities (including disabled submodules), current composer-installable Recipes, and maintained contrib before accepting the narrow custom remainder. Recipe discovery must go beyond packages already present on disk and record support, security-advisory coverage, version, and Drupal/core compatibility so obsolete or unsupported projects do not become recommendations. Contrib records must include the version checked, Drupal/core compatibility, and current maintenance, security, and adoption evidence; adoption is a selection signal, not an automatic veto. A stage with no candidate still needs dated discovery evidence. The final custom decision needs a named, dated accepter, the narrowest scope, a revisit trigger, and evidence. One extension-wide sentence saying alternatives were reviewed is not sufficient.

The filesystem scan follows top-level symlinked extension roots but stays inside each resolved extension boundary, detects cycles, and fails on broken/unreadable/escaping entries or scan caps. Each root must contain exactly one top-level `*.info.yml`; its filename and declared type are the machine-name/type source of truth and must match the directory and custom collection. The existing machine-readable `customCodeInventory.extensions` also records the verifier-computed source-name lint from exact source hostname/site-name boundary tokens. A source-name match needs a named, rationale-backed, packet-local accepted exception there; do not create a second Markdown custom-code inventory.

The live verifier also inventories runtime-relevant custom source files and stable entry-point surfaces: procedural functions/hooks, PHP classes/plugins/subscribers, individual service and route registrations, config schema and shipped config, Twig templates, Drupal JavaScript behaviors (including `.mjs`), stylesheets (including Sass), and SDC components. Tests, fixtures, build scripts, vendor trees, tooling, generated output, minified bundles, source maps, and binary assets are excluded. Every discovered surface must be bound exactly once to a capability of the same extension, and the packet's file hash and surface list must match the live filesystem. This makes catch-all behavior visible; it does not pretend static analysis can determine whether two behaviors are semantically the same capability. Maintainers must reject vague groupings, and mixed files should be split or carry a specific cohesive responsibility.

`G-CODE-01` adds conservative native-first checks without removing any `G-DRUPAL-01` requirement. Path/alias/title/Media-name values used as presentation identity and bare Media/File image render arrays are findings, not automatic semantic verdicts. Remove them in favor of routes, fields, entity references, view modes, Media formatters, or config; otherwise bind a named evidence-backed false-positive or narrow custom exception. An accepted output-affecting exception must have focused tests whose executable source contains non-tautological assertions and machine-detectable access/output/cache signals. The verifier runs those live-discovered PHP tests itself with a fixed PHPUnit command and ignores packet command text as authority.

Config schema checks include inactive install/optional YAML and active core config entities that embed custom plugin IDs, not only config whose root name starts with the extension machine name. PHP lint, PHPCS, PHPStan, and PHPUnit run in the detected DDEV/container runtime with fixed file and time bounds so host PHP drift and unbounded scans cannot create a false result.

Also record which editable content or config drives custom behavior, which platform guarantees it must now handle directly, and what evidence proves it preserves access, cacheability, sanitization, validation, and editor workflow.
...
- literal internal paths/aliases in theme Twig/PHP, metadata injection in theme PHP, hand-written search forms, or unsuffixed `views-*.html.twig` base-template overrides in a custom theme;
...
The verifier's bounded Twig/PHP theme scan is an ownership inventory, not a lint rule that automatically rejects theme code. It excludes `tests`, `fixtures`, and `test-data` directories, detects common text-search controls and search-like literal or Twig `path()` actions/labels/classes, and treats any unsuffixed `views-*.html.twig` filename as a global base override while excluding `--` template suggestions. Every finding references a declared extension capability and must be reviewed as `replace_with_drupal_owner`, `accepted_theme_exception`, or `false_positive`. The first is a pending remediation, not an acceptance: it blocks completion until the code changes and the runtime scan no longer produces the finding. Link every disposition to `off-road-inventory.md`; do not silence findings merely to make the inventory empty.
...
Component registration does not prove a usable public rendering owner. On an anonymous rendered route, verify that every selected Canvas/component provider's required CSS and JavaScript libraries are attached and effective under the active public theme. A standalone custom theme does not inherit another theme's component utility CSS by implication; it must deliberately attach compatible provider libraries, provide equivalent styling, or use components whose asset contract it owns. In `browser-evidence.json`, bind the public route and active theme to each provider and component ID, record the declared library contract, browser-observed asset URLs, direct/equivalent-library mapping, and passing computed-style checks for CSS or interaction checks for JavaScript. The live verifier compares the declared theme with live `system.theme:default`, reconciles every CSS/JavaScript URL with actual `<link rel="stylesheet">` and `<script src>` markup on the fetched route, and requires effectiveness evidence to be a route-bound packet-local browser artifact. A no-library component still needs a rationale and visual browser check. Config, registration, or a free-text observation alone is not evidence that an asset is loaded or effective.
...
- inventory every custom route and controller from the filesystem and live router, including callback- and attribute-defined routes whose controller, form, title, or access callback belongs to custom code. Declare an HTTP method and representative parameters, generate and match a real Request, complete parameter conversion, and run request-aware anonymous access. Record live requirements, effective allowed/denied access, and the generated concrete path. Bind anonymous-allowed routes to the public-only route matrix; bind denied routes to the exact structured private-route inventory with owner, rationale, anonymous behavior, and packet-local evidence. Run bounded verifier-owned PHP syntax, Drupal PHPCS, project-configured PHPStan, config-schema, and focused PHPUnit checks inside the detected project runtime; never execute a command supplied by the packet. An unsupported quality check needs a named maintainer exception with concrete evidence, while a load-bearing test must have a real verifier-owned passing run; a generic statement that custom code was reviewed is insufficient.
...
Every accepted recurring entity/bundle owner needs exactly one future-detail disposition. Entity-display and Canvas detail owners use a Pathauto pattern or explicit editor-supplied alias policy. Row-only/no-detail owners use `canonical_detail_route_denied`; that deliberately narrow disposition proves only that a published probe has no canonical entity route, or that its canonical GET is anonymously denied and has no alias. It does not claim that every possible View or custom alternate path is absent; those public paths remain governed by route discovery and the public route matrix. For public alias policies, live-load distinct existing and editor-created probe entities, verify both bundles and aliases, require anonymous access plus final HTTP 2xx at both paths, confirm the declared Pathauto pattern is enabled/applicable/selected when used, and mechanically compare their URL structure. The accepted browser create workflow's `createdContentPath` must exactly equal the live probe alias. Singleton bundles outside the collection ledger are not pulled into this rule.
...
Rendered title hygiene is keyed to live `system.site:name`: two adjacent delimiter-separated copies of that exact site name at the end of `<title>` are a hard failure. Repeated non-site words and generic length/hash heuristics are not used.
