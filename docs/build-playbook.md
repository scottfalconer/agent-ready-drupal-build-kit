...
Keep every source form/search route in primary coverage. The live verifier binds rendered source and target native controls by privacy-safe form/control ordinal and compares source accessible identity and occurrence when present, control kind, visible labels, required/disabled state, checkbox/radio selection defaults, and ordered `<select>` option labels and states. A target may repair an unlabeled source control at the same ordinal, but missing, extra, reordered, or still-unlabeled target controls fail. The verifier deliberately excludes non-visible CSRF/honeypot controls and retains no internal names, visitor-entered/current input values, or option values; a submit/reset/button value contributes only its rendered public label. Do not leave dependent or AJAX-populated selects empty and call that parity: expose a deterministic populated state at the exact captured route/query state. Empty option evidence or evidence that exceeds the fixed browser bounds blocks completion.
