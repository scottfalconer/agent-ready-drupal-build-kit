...
- Use content types for recurring content patterns and fields for the specific pieces of information editors manage. Time-boxed notices, alerts, and banners are recurring content patterns, not one-off blocks.
- Store filterable or sortable values as typed fields and map source semantics to the specific type: phone numbers -> `telephone`, event start/end and recurrence -> `smart_date` or `daterange`, postal addresses -> `address`, external URLs -> `link`, controlled language -> taxonomy, relationships -> entity references. Avoid burying product attributes, prices, dates, regions, categories, approvals, or relationships in formatted body text or plain string fields. Duplicate node titles inside one bundle are a cardinality smell: model a multi-value field or reference instead of cloned nodes.
...
Recipe-backed assembly follows the same rule. Before creating a custom content type, View, workflow, or cross-cutting feature, discover available Drupal CMS recipes in the target. When a maintained recipe matches the source pattern, treat it as the default owner and record whether it was applied, rejected, blocked, or not applicable in `recipe-start-point.md`. Discovery does not end at the on-disk `recipes/` directory: if a named `drupal_cms_*` candidate is absent, check whether `drupal/<candidate>` is composer-installable (`composer show -a 'drupal/<candidate>'` or its Packagist page) before recording it blocked, and record the upstream availability with the decision. A maintained recipe package is preferred over hand-rolled equivalent config, so every blocked upstream-available candidate must appear in `open-decisions.md` as a composer-require-versus-hand-rolled-overlay decision — batched with any other guardrail-blocked installs into one early human approval question, never silently downgraded to custom config. Custom overlays should be bounded additions on top of the selected recipe or explicit fallback when recipe discovery proves no fit.
