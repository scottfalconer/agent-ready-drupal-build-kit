...
The runner never reinstalls or imports into the working target. It reads that target before and after the run, provisions only a UUID-named `agent-ready-repro-*` project in an independent temporary clone, and deletes only a clone whose ownership marker, generated DDEV name, and real DDEV app root all match the current run immediately before cleanup. Exact-`HEAD` provenance is not treated as an execution sandbox: before `ddev start`, the runner rejects every project compose override and custom command file, any active `hooks` or `remote_config` key, YAML anchors/aliases in DDEV config, and every symlink in `.ddev`. Unsupported customized DDEV projects block rather than being heuristically blessed. If either configuration safety or live identity cannot be proven, the run fails closed and retains any started owned temporary target for manual inspection. Its one intentional working-tree write happens after the after-readback: `review-packet/evidence/reproduction-verification.json`.
