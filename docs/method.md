...
- Every declared collection row requires source/target count reconciliation, Drupal ownership, and non-admin editor add-a-row evidence. Counts must match unless a named owner accepts a specific evidence-backed exclusion; private/unreachable claims require evidence.
...
- Actual composition ownership must match the declaration or have a target-bound accepted deviation with named acceptance and evidence. Rendered SEO `not_applicable` requires reviewed rationale and evidence.
- Blind-review `accepted_out_of_scope` requires a named accepter, reason, and evidence. External blockers leave completion blocked and do not substitute for route coverage.
