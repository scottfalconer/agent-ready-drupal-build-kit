...
- `assembly-verification.json` from the optional launch-only `G-ASSEMBLY-01` runner. It proves a provenance-bound first application, exact no-op reruns, live-applicable extension survival, and tested restoration after an observed partial failure entirely inside an exact-HEAD disposable clone. It is explicitly non-authoritative for the default handoff verdict. See [disposable-assembly.md](disposable-assembly.md).
...
- `G-ASSEMBLY-01`: when launch claims rely on a repeatable assembly workflow, require the separate exact-HEAD pre-assembly runner. It rejects arbitrary command surfaces and unowned deletes, independently reconciles dry-run rows, requires first-run change plus immediate no-op convergence, injects fixtures on live-applicable node/Canvas/menu/alias/View/sitemap surfaces, and proves exact restoration after an observed partial failure. It runs zero working-target Drupal commands and does not participate in the default handoff verdict.
