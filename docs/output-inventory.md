...
- `G-CONTENT-03`: prove required recurring-content fields retain their source semantics. Stable rule IDs bind source-applicable counts to independent target checks and Drupal readback; population, value, ordering, reference-integrity, and non-placeholder violations block completion.
- `G-MEDIA-01`: reconcile reachable source assets to managed Media, evidenced provider-owned/embed behavior at a non-source provider origin, or item-level blockers. First-party assets cannot be relabeled as external-provider content; unresolved assets, incompatible role assignments, and unexplained duplicate placements block completion.
