...
- `G-CONTENT-03`: prove required recurring-content fields retain source-derived semantics. Source requirement IDs map one-to-one to target rule IDs and exact entity/bundle/field/comparison/operator/applicability identities; evaluated/passing IDs reconcile to actual Drupal entities, and every exception is item-level. Unexplained zero applicability and all semantic violations block completion.
- `G-MEDIA-01`: reconcile source assets by observed origin and delivery mechanism to actual managed Media IDs and fields/Canvas props, genuine hosted providers and provider-capable fields, real theme assets, excluded tracking, or item-level blockers. Unreachable assets, generic file/CDN relabeling, incompatible roles, inventory-count drift, unresolved assets, and unexplained duplicates block completion.
