...
- `G-CODE-01`: when custom modules or themes exist, bind the verifier-owned phase-A inventory fingerprint and every stable lexical/registration source-surface ID to one capability in `drupal-readback.json.implementationQuality.customCodeInventory`. Each capability uses a verifier-derived stable ID, stable `AC-` acceptance criteria, and the ordered core → installed Drupal CMS → Recipe → maintained contrib → custom-exception ladder with packet-local evidence. Every load-bearing AC binds to an exact same-extension verifier-inventoried PHPUnit `TESTMETHOD-` identity. The realpath-safe inventory, PHP syntax, required Drupal+DrupalPractice PHPCS, recognized-config-only PHPStan, strict file-reconciled analyzer JSON, redacted diagnostic digests, independently re-resolved HTTPS-dist Composer executable/autoload closure and generated-autoload fingerprints, exact-HEAD disposable DDEV project/database, verifier-generated minimal DDEV config, measured working-target snapshots, identity-safe partial-start cleanup, strict PHPUnit XML/JUnit totals, direct-argv execution, bounded packet JSON, and all aggregate/per-command limits fail closed. `vendor/bin` wrappers and the working target source/runtime/database are never used for analyzers or focused tests; applicable custom-PHP verification requires host-side DDEV orchestration and fails closed inside the working container. PHPStan binds exact requested paths without claiming per-file processing evidence absent from its JSON. Typed quality and focused-test results bind to the phase-A inventory and live evidence. Enabled routes and config still reconcile through Drupal routing/access/schema APIs; inactive optional config and disabled extensions do not acquire false live claims. Cacheability/invalidation and mutable identity are explicitly outside this phase.
