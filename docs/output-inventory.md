...
- `G-ROUTE-01` through `G-ROUTE-06`: cover the full public/anonymous source boundary, browser-expanded routes, repeated-item counts, route drift, fetched public target-required routes, front-page behavior, aliases, unexpected public routes, and Starter cleanup. Denied/admin/private routes live in the exact structured private inventory and never satisfy public route coverage. A packet expectation that repeats a live `5xx` does not make that response acceptable.
...
- `G-COMPOSITION-01`, `G-COMPOSITION-02`, and `G-CANVAS-01`: declare each flexible page's authoring owner and prove the actual target owner/component model matches, or record a target-bound accepted deviation with named acceptance and evidence. Canvas-owned public routes also bind provider/component IDs to live `system.theme:default`, reconcile declared CSS/JavaScript URLs with fetched `<link>`/`<script>` assets, and resolve computed-style/interaction evidence to route-bound packet-local browser artifacts.
...
- `G-DRUPAL-01`: compare configured field widgets/formatters against Drupal plugin `field_types` plus class applicability, distinguish registered extra fields from stale typed components, inventory every active sync-directory Canvas content template (including untracked config) against exact public and embedded display ownership, require stable-ID criteria and one structured core/CMS/Recipe/contrib solution ladder per custom capability, live-reconcile every custom source file/surface and bind it plus every route/controller/theme finding to its extension's stable `capabilityId` and exact `sourceSurfaceIds`, keep replacement findings blocking until removed from the runtime scan, inventory YAML/callback/attribute custom routes through the live router, match and convert a declared representative Request before request-aware anonymous access, run verifier-owned custom-PHP quality checks or require named unsupported-tool exceptions, and live-prove alias or no-public-detail behavior for each accepted recurring entity/bundle owner.
- `G-CODE-01`: retain all custom-code checks under `G-DRUPAL-01` and expose their result separately. Inventory exact custom source/test bytes; run bounded PHP syntax and focused PHPUnit commands inside the detected project runtime over live-derived paths; use Drupal's schema checker on active, embedded-plugin, install, and optional config; detect path/alias/title/Media-name presentation identity plus bare Media/File image render arrays; and require exact source bindings, Drupal-owner replacement or evidence-backed exceptions, machine-detected test coverage, and live verifier-owned test success for load-bearing custom behavior. Packet command/status text is never execution authority.
...
The installed skill's `scripts/verify.mjs` is the default target-local verifier. It binds the packet to the detected DDEV runtime by target origin, matching Drupal `system.site` UUID/name, front-page setting, config-sync directory, and clean config status; independently requires real Git-tracked YAML in that current sync directory while scanning the full active sync directory for Canvas contradictions; audits live field widget/formatter resolution and custom runtime code; fetches public primary/target-required routes and separately verifies structured private-route behavior; rejects non-success public responses even when the packet reports the same `5xx`; checks each fetched public route's rendered canonical, meta description, `og:image`, and exact live-site-name title suffix; runs semantic packet-readiness checks; and writes `review-packet/evidence/live-verification.json`. Its success does not replace authenticated editor/browser evidence, independent verification, or blind review.
