...
8. For stronger maintainer or launch evidence, optionally reproduce the build from immutable inputs in a disposable target, never the working target. Record `reproduction-evidence.json`: clean dependency install/provision/config import, canonical content and file restoration, an inert six-phase transcript, stable ID/count/config/file/route readback, and byte-identical working-target identity. The linter reports only non-authoritative `E-REPRO-01: evidence_recorded`; it never executes packet commands and this record does not affect completion authorization.
