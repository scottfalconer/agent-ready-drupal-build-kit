...
6. Test anonymous public routes and a realistic non-admin editor workflow. Every custom public bundle and every bundle that owns repeating public content needs this workflow. A representative editor must be able to add or change recurring content and see it on the expected public route without code changes. Independently falsify each load-bearing field and each field claimed to affect anonymous output. For recurring public models, inventory date/year/season/period/taxonomy dimensions and complete `next-cycle-verification.json`: use the proven least-privilege role for a future-cycle publish/public-output probe and prove residue-free cleanup, or record structured N/A only after machine-backed discovery finds no such dimension. The default verifier independently reconciles N/A against a metadata-only census of the active Drupal field, option-list, taxonomy, and workflow configuration.
