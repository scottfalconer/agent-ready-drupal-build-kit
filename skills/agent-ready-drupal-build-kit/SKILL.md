...
8. Reproduce the build from immutable inputs in a disposable target, never the working target. Record `reproduction-evidence.json`: clean dependency install/provision/config import, canonical content and file restoration, an inert six-phase transcript, stable ID/count/config/file/route readback, and byte-identical working-target identity. A snapshot restore is recovery evidence, not clean reproduction.
