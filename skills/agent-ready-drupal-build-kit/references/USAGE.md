...
The default verifier exits zero only when the detected live DDEV target, packet readiness, Drupal site identity, independent verification, and blind review all authorize completion. It fetches every accepted route and target-required public route, inspects links present in server-rendered response HTML, requires discovered same-origin targets to be declared or exactly dispositioned, validates exact expected external redirects without fetching the external origin, blocks direct links back to the original source origin unless the exact referrer/target pair has a rationale, named accepter, and packet-local evidence, rejects self-consistent `5xx` failures, inspects response canonical/meta-description/`og:image` output on primary routes, and independently requires real Git-tracked YAML in the current config-sync directory. It does not execute JavaScript; browser-only links must be discovered through browser-first route expansion and represented in the route matrix. Every discovered route role needs a representative primary route, so listings cannot hide broken detail pages. An explicit target must match the detected DDEV origin. Packet-only data and injected test runtimes may help diagnostics, but can never authorize completion. Exit `2` means structurally valid but incomplete; exit `1` means invalid packet or live-target checks. This local verdict is not production or launch approval.
