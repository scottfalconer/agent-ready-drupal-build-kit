...
- unfiltered Drupal readback, generated by the kit's `scripts/generate-readback.mjs` rather than hand-written, for the `system.site` UUID, front page, config sync directory, all nodes including unpublished/default content, per-bundle published/raw counts, all aliases including duplicates, menus/menu links, Canvas pages when available, media counts, themes, and config status;
