...
- For every accepted collection, declare the source and target continuation modes as `paged`, `load_more`, `infinite_scroll`, or `none`, plus each observable page size. Bind the target to one exact live View display unless an evidence-backed documented exception proves that the source is a genuine single-page collection. A paginated source needs an exact target continuation request, a captured `publicRouteChecks` row that makes the live verifier fetch that request independently, evidence that it is behaviorally equivalent, and a successful item-distinct second state; it cannot be relabeled as `none`. The target page size may differ from the source when the continuation behavior remains equivalent. A `none` declaration is valid only for a genuine single-page collection whose reconciled rendered/entity counts prove that nothing was truncated.
...
- For View-owned collections, read the effective pager from the live display, including inherited versus overridden configuration, pager plugin type, items per page, and offset. A fixed-limit `some` display, a nonzero fixed offset, or a page-size-only slice without a real continuation is not pagination.
...
- every accepted collection pagination declaration, including exact live View pager readback and, when continuation exists, a captured public-route state that the live verifier fetches independently and whose item identity differs from the initial state;
...
- Collection pagination complete: every accepted collection declares source/target mode and observable page sizes, the live View pager matches the target declaration with zero offset, and every continuation-capable collection has an exact successful item-distinct second-state check.
