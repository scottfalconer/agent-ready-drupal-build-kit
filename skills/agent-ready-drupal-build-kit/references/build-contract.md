...
- Inspect the active Git-tracked config export for enabled `canvas.content_template.*` config before declaring Canvas intentionally unused or assigning presentation ownership to a theme entity template. Bind each template to an exact public entity type, bundle, and view mode, including non-node and embedded public displays; honor explicit row-only/no-public-detail ownership instead of assuming every bundle's `full` display is public. An enabled Canvas content template owns rendering for its targeted public display and supersedes the normal theme entity template there. Disable or delete the active template, or declare and verify Canvas as the real owner; theme CSS or Twig that merely exists is not ownership evidence.
...
- Audit every configured form widget and view formatter against the live field definition. The configured plugin must exist, be applicable to that field type and target entity, and resolve without Drupal silently substituting a fallback plugin. Any referenced target view mode must also exist. Exclude components registered by Drupal's extra-field API from widget/formatter checks while continuing to reject unregistered stale typed components.
- Give every URL-capable recurring entity bundle with a real public detail owner a future-content alias policy, including non-node entity types. Live-load a representative entity, verify its bundle, verify an enabled applicable Pathauto pattern when selected, and prove both directions of alias-manager resolution for the new alias. Imported aliases alone do not prove future editorial correctness. A `view_row` or other explicitly row-only collection has no detail alias obligation.
...
- Audit every configured form widget and view formatter through Drupal's plugin manager: the plugin must declare the field's type in `field_types` and its class `isApplicable()` check must pass. A typed display component without a matching field definition or registered extra-field definition is a configuration violation; Drupal silently choosing a fallback renderer does not make the exported config valid.
...
- On each anonymous public route owned by Canvas/components, prove the selected component providers' required CSS and JavaScript libraries are attached and effective under the active public theme. In `browser-evidence.json`, bind the route, active theme, provider, and component IDs to the declared library contract, browser-observed asset URLs, and passing computed-style (CSS), interaction (JavaScript), or no-library visual checks. Equivalent theme assets need explicit mapping evidence. The live verifier must match the theme to live `system.theme:default`, match every declared asset URL to fetched `<link>`/`<script>` markup, and resolve effectiveness evidence to a route-bound packet-local browser artifact. Component registration, rendered markup, fake URLs, or free text are not styling evidence.
...
- Inventory custom modules, custom themes, runtime-relevant source files/surfaces, routing files, controllers, and tests from the live project filesystem and live Drupal router. Include callback- and attribute-defined routes when their controller, form, title, or access callback belongs to a custom extension. For each distinct custom capability in an extension, complete a separate structured `solutionLadders` record with a stable `capabilityId` in `drupal-readback.json`: exact need and stable-ID acceptance criteria; Drupal core; installed Drupal CMS capabilities, including disabled submodules; current composer-installable Recipes with Drupal/core compatibility plus support/security status; maintained contrib with Drupal/core compatibility and maintenance, security, and adoption evidence; and a named, dated reason the narrow custom scope remains. Every partial candidate must reference the exact unmet criterion IDs. A Recipe's absence from the current filesystem is not evidence that it is unavailable, and one catch-all ladder or a free-text claim that alternatives were reviewed is not sufficient. Every discovered source surface must be bound exactly once to an extension capability; every custom route, discovered controller, and theme ownership finding must reference that `capabilityId` and the exact implementing `sourceSurfaceIds`. A controller without a route remains governed by that capability review. The scanner makes hooks, classes, individual registrations, config schema, Twig, JavaScript behaviors, CSS/Sass, and shipped config visible but cannot infer semantic cohesion; split mixed files or give each surface a specific responsibility. Tests, fixtures, scripts/tooling, vendor/generated files, minified bundles, maps, and binaries are excluded. For every custom route, declare an HTTP method and representative parameters, then generate, match, convert, and access-check a real anonymous Request; neutral access is not a final disposition. The verifier runs canonical Drupal PHPCS and project-configured PHPStan checks when those tools are supported; packet-provided commands are never executed, and an unavailable check needs a named evidence-backed maintainer exception. Every anonymous-allowed custom route must appear in the public-only route matrix. Every denied custom route must instead bind to the exact structured private-route inventory with owner, rationale, anonymous behavior, and packet-local evidence. Both carry explicit access, cacheability, sanitization, dependency-injection, test, and off-road review. Packet prose that says no custom controller exists cannot override discovered code.
- Treat `G-CODE-01` as the custom-code subset of the broader Drupal implementation gate; it does not weaken `G-DRUPAL-01`. The live verifier runs PHP syntax checks and Drupal's config-schema checker from fixed verifier code, never from packet-supplied commands. It conservatively finds mutable alias/title/Media-name identity used to select presentation and hand-built Media/File image render arrays that can drop entity access or cacheability metadata. Every finding must bind to its exact source surface and be removed in favor of a Drupal owner, accepted as a tested narrow exception, or identified as a packet-evidenced false positive.
- Mark each custom capability explicitly as load-bearing or not, with rationale. Custom routes and accepted output-affecting exceptions are load-bearing. Bind them to assertion-bearing test files from the live filesystem and record focused passing evidence for the machine-required checks, including anonymous behavior, identity mutation, access, cacheability dependencies, and cache invalidation where applicable. A command recorded as test evidence is data only; the verifier must not execute it.
...
Do not collapse multiple custom capabilities into one extension-wide decision or prose sentence. Give each capability and each acceptance criterion a stable ID, then record candidates and evidence at each step in this order: core; already installed and disabled Drupal CMS capabilities; current composer-installable Recipes; maintained contrib; then the narrow custom exception. Search beyond Recipes already present on disk, and reject unsupported or obsolete packages rather than treating a familiar name as a current recommendation. If a stage has no candidate, say so explicitly and record how it was checked. A partial candidate must list the exact unmet criterion IDs. Revisit or remove custom code when a selected native owner can now meet the need.

Mechanically inventory review-worthy ownership signals in custom theme Twig/PHP outside test/fixture data: literal internal paths or aliases, theme-PHP metadata injection, hand-written search forms, and every unsuffixed `views-*.html.twig` base-template override. Each finding must reference its extension's `capabilityId` and be dispositioned with evidence and an `off-road-inventory.md` reference. `accepted_theme_exception` and reviewed `false_positive` findings may pass; `replace_with_drupal_owner` is pending implementation and blocks completion until the runtime scan no longer finds it.

For every custom route, resolve the filesystem-declared route name through the live bootstrapped Drupal router. Evaluate its representative parameters as the anonymous account through Drupal's access manager and record the resulting allowed/denied/neutral disposition. Bind only anonymous-allowed routes to `route-matrix.json`; bind denied routes to `drupal-readback.json routing.privateRoutes`. Do not infer anonymous access from a familiar permission string; `_role`, arbitrary permissions, custom access callbacks, and parameter conversion must use Drupal's actual access system.
...
- active Canvas content-template readback: scan the full active sync directory, including untracked/ignored YAML, and deterministically inventory top-level `canvas.content_template.*` config, its tracked status, enabled status, entity/bundle/view-mode target, and whether that target is public. A Canvas-unused claim must have no enabled template targeting public output.
