...
For every custom route, resolve the filesystem-declared route name through the live bootstrapped Drupal router. Evaluate its representative parameters as the anonymous account through Drupal's access manager, record the resulting allowed/denied/neutral disposition, and bind its generated concrete path or route name to `route-matrix.json`. Do not infer anonymous access from a familiar permission string; `_role`, arbitrary permissions, custom access callbacks, and parameter conversion must use Drupal's actual access system.
...
- tracked Canvas content-template readback: deterministically inventory top-level active-sync `canvas.content_template.*` YAML, its enabled status and entity/bundle/view-mode target, and whether that target is public. A Canvas-unused claim must have no enabled template targeting public output.
