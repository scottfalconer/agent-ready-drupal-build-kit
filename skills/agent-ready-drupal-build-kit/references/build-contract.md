...
- For every list, grid, schedule, directory, archive, catalog, feed, gallery, or search-like route, produce a collection ownership ledger entry: source route or brief requirement, collection pattern, source/brief and target item counts, Drupal entity/bundle owner, required fields, View or collection owner, detail route owner, and non-admin editor add-a-row evidence. Every declared ledger row must contain all three kinds of proof: reconciled counts, real Drupal ownership, and a successful editor add-a-row task. A recurring/query-backed collection declares `collectionSelection: query_backed` and uses a View; an unlimited entity-reference list is valid only as `curated_reference` when packet-local source or preserved-brief evidence plus editor evidence proves deliberate selection or ordering and the ledger records a curation rationale. Body markup, a one-off blob field, or static cards are failed collection owners unless the packet records a genuine one-off exception. Detail pages, individual node routes, or sample items do not satisfy the collection-route gate; a source listing/archive route needs its own View, redirect, intentional-drop disposition, or documented owner decision.
- For every accepted collection, declare the source and target continuation modes as `paged`, `load_more`, `infinite_scroll`, or `none`, plus a positive observed page/batch size on each continuing side. Declare each continuation mechanism independently as an exact `request` or `browser_interaction`. Request-addressable states need exact distinct source and target requests bound as a same-route query state or verifier-discovered continuation link; the verifier fetches them and requires distinct visible-text or media semantics. A JS-only state may be recorded diagnostically but remains `review_required` and cannot authorize machine completion until verifier-owned replay exists. Bind the target to one exact live View display unless an evidence-backed documented exception proves that the source is a genuine single-page collection. A continuing source cannot become `none`, and a `none` source cannot acquire target continuation. A source `load_more` or `infinite_scroll` experience may be replaced by `paged` or `load_more` only through an exact `acceptedBy` name, rationale, and packet-local evidence in an `accessible_continuation_replacement` disposition; source and target page sizes may differ. A `none` declaration requires null page sizes, `none` mechanisms, no continuation requests/state IDs, verifier-owned falsification of obvious continuation signals, and reconciled rendered/entity counts proving that nothing was truncated.
...
- Views own public collections and editor-maintained listings. A landing page may compose a View-backed recurring collection, but must not impersonate one recurring item or carry an unlimited manual reference list merely to render the collection. A hand-coded card grid, controller array, or Canvas page that must be manually updated for each new item is a modeling failure unless the source or preserved brief proves intentional editorial curation or a truly one-off pattern.
- For View-owned collections, read the effective pager from the live display, including inherited versus overridden configuration, pager plugin type, items per page, and offset. Treat enabled Views blocks placed through block layout, Canvas registration, or bounded Layout Builder sections as live public display registrations. A fixed-limit `some` display, a nonzero fixed offset, or a page-size-only slice without a real continuation is not pagination.
...
- If implementation cannot match the declared model, do not silently degrade to a fake pass. The actual target owner must match the declared owner for machine completion. A target-bound deviation may document the blocked owner/model, actual fallback owner, reason, reviewer-visible consequence, recorded accepter label, and live evidence for human review, but remains completion-blocking; the local packet does not authenticate that accepter.
...
- Match the source site's visual language with a custom theme/subtheme, Drupal blocks, regions, templates, image styles, and CSS. Choose image and responsive-image styles whose delivered raster dimensions meet or exceed their object-fit-aware painted dimensions at the verified viewport; do not stretch a small thumbnail style into a blurry card, listing, event, news, hero, or detail image. Do not fall back to Olivero or another stock theme as the final public experience unless the source itself is generic enough to justify that choice.
...
- verifier-owned identity and gross-omission checks for every accepted source/target route, not only primary visual samples: require and live-bind each route row's `sourceStatus` and `sourceFinalPath`, live-bind each HTML route row's source/target title and H1, preserve declared exact path-plus-query identity, and fail complete loss of source forms, behavior-bearing embeds, material headings/media, or most substantive visible text. Route semantics remove inert and explicitly hidden subtrees before selecting the first surviving `<main>` or `[role=main]`, falling back to the response body only when the original response declared no main region. Class-driven CSS visibility remains a browser-evidence concern. Explicitly declared non-HTML routes remain status/content-type checked without fabricated HTML identity requirements. An intentional title/H1 change needs one narrow route-level `identityChangeDisposition` bound to the exact current source/target title/H1 values and exactly one matching passing independent-verification counterpart; it cannot waive semantic omissions;
...
- every accepted collection pagination declaration, including exact live View pager readback, positive observed source/target batch sizes for continuing collections, verifier-owned route-bound final-request plus semantic distinctness for exact request-addressable states, and a `review_required` blocker for browser-interaction-only continuation the verifier cannot replay;
...
- composition model fidelity for flexible landing-like routes, including proof that the actual target owner matches the declared owner, section ownership, expected editor actions, and public-output proof; target-bound deviations remain diagnostic human-review records and cannot authorize machine completion;
...
In source-site mode, the managed-browser source/target floor also compares the primary navigation as a semantic tree at desktop and mobile widths. It preserves bounded DOM order, link/control kind, depth and parent relationships, normalized hrefs, and label identities; it applies exact accepted source-to-target route mappings before comparison. Query strings and labels are retained only as SHA-256 identities in finalized evidence. A swapped or invented label, lost hierarchy, changed order, omitted/added entry, or wrong href blocks completion even when every destination happens to resolve. The capture is bounded to 128 entries, 160 normalized characters per label, and depth 8; exceeding a bound is incomplete evidence, not permission to truncate the menu silently.

An intentional editorial difference can be accepted only with one `route-matrix.json.primaryNavigationParityDispositions` record using `schemaVersion: public-kit.navigation-parity-disposition.1` and exactly one matching passing `independent-verification.json.primaryNavigationParityDispositionChecks` row. Changed hrefs are mapped only after verifier-owned redirect materialization proves the exact source/target association. Copy the exact viewport fingerprints and sorted `differenceKinds`, then add named acceptance, rationale, and packet-local evidence. Broad approval, duplicate or stale rows, unproven mappings, or missing evidence cannot waive the mismatch.
...
- Views owns dynamic listings, directories, filters, related-content blocks, search-like pages, homepage collections, taxonomy landing displays, and editorial/admin listings. A composed landing page embeds or places that View; it does not become one recurring item or replace a query-backed collection with unlimited manual entity references. Curated references are reserved for hand selection or ordering proven by the source or preserved brief plus editor evidence, with an explicit rationale and packet-local evidence.
...
- scoped gap list;
- off-road inventory;
- maintainer review packet with a separate human-facing binary stake-my-name status.

Gate records, with machine evidence, pending human status, or blocked stubs as applicable:
