...
- Complete `review-packet/negative-route-consent.json`. Declare the missing-route noindex policy, access-wall routes, legal/privacy scope, and every active consent manager/application with its enabled/required state and controlled resources. For every primary route, capture before-consent browser evidence in a fresh context after clearing consent storage, but treat that packet record as diagnostic rather than machine authority. Until the kit performs its own browser/network capture, optional or disabled controlled resources remain blocked. A rendered broken legal/privacy link is implementation work and cannot be deferred as production-only.
