...
### Assembly Rerun Safety Record

Every rebuild has an assembly path, whether it is one command or a recorded sequence of Recipe, config, import, evidence, cleanup, and site-setup steps. Before rerunning that path after editors or extensions have changed the site, exercise it safely in disposable state. Record the builder-observed result in `review-packet/assembly-evidence.json` and store raw plans, inventories, logs, and restoration records under `review-packet/evidence/assembly/`.
...
The packet linter reports a structurally complete record as `E-ASSEMBLY-01: evidence_recorded`. That label is deliberately non-authoritative: the verifier checks record shape, local references, and hashes, but does not execute packet-provided commands or independently observe the disposable rerun. The record therefore cannot authorize or block completion. Treat any missing safeguard as unresolved maintenance risk until a future verifier-owned runner can execute a bounded assembly contract safely.
...
- assembly rerun safety evidence (`E-ASSEMBLY-01: evidence_recorded` when structurally complete; never completion-authoritative).
