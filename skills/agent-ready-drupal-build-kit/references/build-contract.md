...
For each public route check, record target URL/final URL, viewport, target screenshot, title, H1, key visible body intent, section order, header/footer treatment, typography and spacing notes, media placement, functional behavior notes, accepted exceptions, brief requirement IDs when applicable, and pass/fail status. Keep capture-state declarations on that same record: every normalized target request + viewport name + stable state ID tuple is unique, every primary route has a `default` desktop and mobile state, and every state binds a fixture revision, at most 12 interaction steps, coherent menu and consent state, at most 16 exact content-count assertions, and at most 8 selector masks whose declared area is no more than 25% of the viewport. If the structured `default` mobile state declares `menuState.toggleVisible: true`, record it closed and also capture an accepted, passing `mobile-menu-open` record with a target screenshot, bounded interaction step, and observed open navigation. Do not infer that requirement from prose or create a second state manifest. Per-state masks describe authored comparison evidence; they do not authorize or replace verifier-owned global-chrome anchor masks. In `source_site` mode, also record source URL/final URL, source screenshot, and optional diff image or score when the tool supports it; brief mode remains target-only.
