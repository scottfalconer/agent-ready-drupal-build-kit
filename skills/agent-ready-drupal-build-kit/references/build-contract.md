...
- Declare the build type in the pattern map: structured Drupal-native rebuild with Canvas intentionally unused; hybrid structured content plus Canvas composition; Canvas-heavy rebuild with Drupal-owned structured data embedded; or constrained fallback because Canvas was unavailable or blocked. The build type must match the source shape and the editor ownership evidence. A Canvas-unused declaration is falsified by an enabled `canvas.content_template.*` or `canvas.page_template.*` config targeting a rebuild bundle (Drupal CMS ships these enabled by default, and they supersede theme `node--*.html.twig` output): disable or delete the template, or declare Canvas the composition owner.
...
- Run Starter and route drift cleanup before handoff. Check `/home`, `/page/1`, `/privacy-policy`, raw `/node/*` routes, starter Canvas pages, stale menu/footer links, unpublished starter pages, duplicate aliases, and aliases that normalize to the wrong final URL. Remove, redirect, publish, or explicitly block each one. Sweep starter config entities the same way: every open-but-unreferenced webform, Views page, or Canvas content/page template needs a keep, close, or delete disposition with a named owner and rationale in `independent-verification.json` `starterConfigEntitySweep`.
