...
- Browser-first route expansion is mandatory for JavaScript, static-bundle, smart-link, SPA, or app-like sources. Probe likely public slugs found in rendered links, source bundle route data, metadata, sitemap/robots hints, click targets, asset names, canonical tags, social metadata, observed naming patterns, and routes referenced from imported content bodies (record those under `candidateRoutesFromImportedContentBodies`). A curl response alone cannot close the route inventory when browser evidence, bundles, or assets imply more public routes.
...
Also check target-required routes introduced by the rebuild even when they were not explicit source routes: privacy/legal/footer links, sitemap and robots behavior when enabled, login/admin expectations, canonical front page behavior, and any locally introduced menu or footer links. The default live verifier must fetch these routes from the real DDEV target. A broken target-owned footer link or any target-required `5xx` fails the route gate even when packet records report the same status. The live verifier also re-fetches a seeded random sample of the full route matrix (seed recorded in the report) and crawls same-origin rendered links from the front page and sampled content pages. Record every checked menu, footer, and rendered link as a per-link record — never as a bare text label. Same-origin links record `href`, observed `status`, and `finalPath`; links to other origins (state portals, social profiles) record the absolute `href` with `disposition: "external"` and are validated without being fetched. A rendered link that returns 4xx, including an unrewritten link imported inside body content, fails the route gate unless a named owner records an `owner_accepted_broken` disposition with a rationale on that link record.
...
The route matrix must reconcile source-rendered routes against target routes. It must fail the review loop when a source route is missing, a target route renders the wrong H1/body pattern, homepage/front-page behavior is wrong, a legal/footer route is broken, or an unexpected public 200 exists because of duplicate aliases, duplicate content, stale menu links, default demo content, or route-normalization shortcuts. Expected redirects are acceptable only when recorded with source evidence and rationale. When a route row maps a source path to a different target path, the mapping must materialize as a permanent HTTP 301 (or 308) on the target that lands on the mapped path, or the row must carry a `noRedirectDisposition` with `acceptedBy` and rationale; a mapped source path that 404s on the target fails the route gate. Mappings are keyed by the full source path including its query string, so query-differentiated legacy routes (`/page.aspx?ID=1` vs `?ID=2`) are each verified, and duplicate source paths with conflicting targets fail loudly.
