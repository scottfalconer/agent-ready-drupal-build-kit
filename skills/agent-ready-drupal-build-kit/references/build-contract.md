...
- Inventory public source assets by stable row ID, asset class, visible source role, observed source origin, delivery behavior/mechanism, and discovered/reachable/unreachable counts. Every unreachable item needs a stable ID, concrete boundary evidence, and remains a completion blocker. A raw asset count does not show whether a content image, download, hosted embed, icon, tracking pixel, or decorative asset was migrated into an appropriate role.
...
- For every required field in a recurring collection, record a source semantic requirement before mapping the target rule. Bind source route/object, field, comparison field, rule, operator, source-applicable count, and actual evaluated/passing entity IDs exactly through the target rule, independent check, Drupal readback, and entity inventory. Zero applicability needs a named rationale plus packet-local evidence; target exceptions need unique source item IDs, reasons, and per-item evidence. Equal entity counts do not pass when dates lose their ordering, references no longer resolve, values become placeholders, or populated source fields arrive empty.
...
- every source semantic requirement and mapped recurring-content rule, including exact field/comparison identity, target evaluation/pass/violation counts, and one-to-one Drupal readback for the same IDs;
- reachable source-asset reconciliation, including observed origin/delivery behavior, managed Media, genuine hosted-provider behavior, theme assets, excluded non-content tracking, item-level blockers, invalid source-to-target role assignments, and unexplained duplicate placements;
...
- matching item counts with lost field values, broken references, invalid field relationships, or placeholder substitutions;
- collection routes without a Drupal owner plus View/collection config and editor add-a-row evidence;
- reachable source assets represented by unresolved URLs, assigned to an incompatible public role, or duplicated into unrelated presentation fields without a reviewed reason;
...
- Public media inventory reconciled: content, brand, and download assets retain corresponding managed-Media uses through real Media IDs plus entity-reference fields or typed Canvas component props; hosted embeds/services bind observed delivery mechanisms to provider-capable fields and provider origins; icons/decorative assets resolve to real theme files; tracking resolves to excluded non-content ownership; or uniquely identified item-level exceptions apply. Generic file/CDN delivery cannot be relabeled as a hosted provider, unreachable assets block completion, and all ledger/check/readback IDs and counts reconcile to actual Drupal inventories.
...
- Record any unreachable, private, credentialed, or hosted-provider behavior as a scoped gap with the exact missing input.
...
- field-semantic evidence: every required recurring-content field has a source requirement ID and mapped rule ID with exact entity/bundle/field/comparison/operator identity, source-applicable count, expected target pass count, Drupal readback, independent violation count, and item-level exception evidence.
- media-provenance evidence: source asset inventory rows record observed origins and delivery behavior, then reconcile one-to-one to managed Media/reference fields, genuine hosted providers, theme assets, excluded non-content tracking, or named evidence-backed exceptions, with no unresolved assets, invalid role assignments, or unexplained duplicate placements.
