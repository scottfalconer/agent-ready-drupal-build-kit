...
- Reconcile every accepted non-redirect homepage and landing route exactly once across the route matrix, `compositionModel.flexibleLandingRoutes`, `pageCompositionOwnership`, and independent composition-fidelity checks. Keep expensive desktop/mobile review risk-based, but do not let a route escape its composition and editor-owner decision merely because it is not a primary visual sample. Reconcile every declared editor-facing section exactly once with `sectionOwnershipMatrix`; a single global example is not enough. Every Utility Page owner also needs one exact source/target `utilityPageExceptions` row; aggregate labels such as “stable pages” are invalid.
