...
- First-party downloadable assets on the source origin or its CDN (PDFs, documents, images, audio, video) are in-scope file migration, not a provider boundary; only genuine third-party transactional systems stay provider-owned. After import, produce `review-packet/source-origin-dependence.json`: scan every stored file/media/link field table and stored body text for URLs on `sourceBaseUrl` and the asset hosts recorded in `source-audit.json`, localize or rewrite each hit (prefer entity/UUID linking over raw source aliases), and cover any remaining dependence with an external-asset strategy accepted by a named owner. The verifier re-runs these scans against the live database and hard-fails primary routes whose rendered links, assets, or primary CTA point back at the source origin, and routes whose text matches stub/archive boilerplate, unless each stub route is a named accepted exclusion in `scoped-gap-list.md`.
