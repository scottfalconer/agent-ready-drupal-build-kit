...
Alias architecture must be config-owned, not import-script-imposed. Every public bundle needs a record in `pattern-map.json` `displayConfig.pathautoPatterns`: either an accepted Pathauto pattern with its alias prefix (for example `/news/[node:title]` with prefix `/news`), or a manual-alias policy naming an owner and rationale (a policy is scoped to one entity type and bundle). A pattern whose alias prefix is the site root (`/`) needs a recorded `rootPrefixJustification` explaining why the bundle's aliases do not live under a bundle prefix. The packet verifier fails completion for any used bundle with none of these.

Record the alias readback with bundle attribution: `drupal-readback.json` `routing.aliases` needs at least one `{path, alias, bundle}` entry per used node bundle. Unattributed or empty alias readback fails completion, because the alias-structure checks below cannot run against it.

Editor-created content must land in the same alias structure as content already in its bundle. Every declared Pathauto pattern requires an accepted passing create-workflow editor probe that records `createdContentPath` — a bundle without a create probe fails, so relabeling or skipping the probe does not bypass the check. The live verifier compares the probe alias against the bundle's declared alias prefix and the alias structure of existing bundle content, and fails on a prefix mismatch, a structure mismatch (for example a probe landing on `/qa-test-news` while existing content lives under `/news/`), or a raw `/node/{id}` fallback — an import script that writes aliases directly while Pathauto covers only starter bundles produces exactly that drift. Root-prefix bundles are verified against existing bundle aliases, so they cannot pass without bundle-attributed readback.
