...
Keep every source form/search route in primary coverage. The live verifier compares the rendered source and target native controls by accessible identity and occurrence, including control kind, visible labels, required/disabled state, checkbox/radio selection defaults, and ordered `<select>` option labels and states. It deliberately excludes non-visible CSRF/honeypot controls and retains no internal names, visitor-entered/current input values, or option values; a submit/reset/button value contributes only its rendered public label. Do not leave dependent or AJAX-populated selects empty and call that parity: expose a deterministic populated state at the exact captured route/query state. Missing labels, empty option evidence, or evidence that exceeds the fixed browser bounds blocks completion.
