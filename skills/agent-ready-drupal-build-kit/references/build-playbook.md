...
At import time, produce `review-packet/source-origin-dependence.json` from the installed template. Scan every stored file, media, and link field table and every stored body/text field for URLs on the declared `sourceBaseUrl` and the asset hosts recorded in `source-audit.json` `sourceAssetHosts`. Every field the field-output matrix records as a file, image, media, link, text, entity-reference, string, or uri type needs a scan row, and a scan row's `loadBearing` must agree with the matrix's `affectsAnonymousOutput` for the same field. Localize files into Drupal-managed storage, rewrite remaining hrefs (prefer entity/UUID-based linking over raw source aliases), add redirects, or record an explicit external-asset strategy accepted by a named owner; the `external_asset_strategy_accepted` disposition on a scan row is only valid when an `externalAssetStrategies` record with a named `acceptedBy` covers that field.

Use the exact verifier predicate when recording counts, because the default verifier re-runs the same queries via `drush sqlq` against the live database at packet time and blocks completion on any mismatch: `totalRows` is `SELECT COUNT(*) FROM <table>`, and `sourceOriginRows` is `SELECT COUNT(*) FROM <table> WHERE <column> LIKE '%<host>%'` OR-joined across the `sourceBaseUrl` hostname and every `knownSourceAssetHosts` entry. The predicate uses bare hostnames, not full URLs, so plain-text mentions of the domain in body text count as source-origin rows.

Stub findings record one of three treatments: `stub_accepted` (a named owner accepts the stub and a matching row exists in the `scoped-gap-list.md` stub-exclusion table naming the finding's `scopedGapListEntry` ID and the exact route), `content_migrated` (the boilerplate was found and the content was actually migrated; keep the finding as the audit trail), or `route_dropped` (the route was deliberately removed from the target). The live verifier always scans fetched primary routes against a set of kit-default boilerplate phrases (for example "open the original", "archived copy", "content coming soon") in addition to the patterns you declare, so declared patterns cannot be used to narrow the scan.

Gate `G-ASSET-01` fails closed when a load-bearing field is mostly source-origin URLs without an accepted strategy, when a configured file/media field on a declared collection stores zero rows, when a fetched primary route renders links or assets that point back at the source origin, or when rendered text matches stub/archive boilerplate without a named per-route accepted exclusion in `scoped-gap-list.md`. As an informal build-time self-check, a target page whose visible text is dramatically shorter than the browser-rendered source page usually signals unmigrated content.
