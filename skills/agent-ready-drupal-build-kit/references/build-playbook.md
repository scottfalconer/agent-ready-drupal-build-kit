...
After the source and target response censuses pass, the verifier reconciles every accepted route, not only the primary screenshot sample. For HTML it live-binds the declared source/target title and H1, compares source/target identity, and applies a deliberately coarse omission floor: actual source forms and their form-associated controls, behavior-bearing media/embeds, material heading/media structure, and substantive visible text cannot disappear wholesale. All intrinsic counts select the first declared `<main>` or `[role=main]` region before explicit-hidden filtering and fall back to the response body only when no main region exists, preventing shared header/navigation/footer text from masking omitted or hidden main content. Comments, scripts, styles, templates, `noscript`, SVG, and subtrees or standalone elements explicitly hidden with `hidden`, `aria-hidden="true"`, inline `display:none`, or inline `visibility:hidden` do not count. Class-driven or stylesheet-computed visibility cannot be proven from raw response HTML and remains covered by browser evidence. For non-HTML routes it instead requires exact observations and matching normalized response content types. Different HTML markup, hostnames, Drupal tokens, cache-busters, and internal field names do not fail this floor. A deliberate title or H1 change needs one route-specific `identityChangeDisposition` with a named accepter, rationale, and packet-local evidence; that identity-only disposition never accepts a missing form, embed, gallery, or page body. The report retains full-result fingerprints and totals while bounding per-route rows and examples; one consolidated continuation blocker points to the bounded route-specific evidence.
...
A prose plan for listings is not Views parity. Views/page parity needs exported Views config, route/display proof, access checks, sorting, filters, pagination, cache behavior, and browser-rendered evidence. For every accepted collection, record source/target continuation modes, positive observed page/batch sizes when continuing, and whether each continuation is an exact request or a browser interaction. Exact requests are bound through `browser-evidence.json.publicRouteChecks` and independently fetched; the verifier rejects a continuation that resolves back to the initial final request or whose live response lacks distinct visible-text or media semantics. Genuinely JS-only continuation binds an exact non-default interaction-bearing capture state and is reported as self-attested rather than verifier-replayed. Bind one exact live display unless an evidence-backed documented exception proves a genuine single-page collection. The verifier resolves inherited or overridden pager settings from live Drupal, including Views blocks registered in bounded Layout Builder defaults and anonymous-viewable per-entity overrides, and rejects fixed-limit `some` displays, nonzero offsets, incompatible live pager modes, and continuation-to-`none` laundering. An accessible continuation-mode replacement needs an exact `acceptedBy` name, rationale, and packet-local evidence; target page size does not have to equal source page size.
