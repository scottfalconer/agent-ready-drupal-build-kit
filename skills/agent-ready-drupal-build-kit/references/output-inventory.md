...
- `G-PRIVACY-01`: independently probe a guaranteed-missing route, access-wall canonicals, rendered internal legal/privacy links, and active Drupal consent config. For optional or disabled controlled resources—including selector-only and attachment-only declarations—create a fresh isolated Chrome context per primary route, record CDP `Network.requestWillBeSent` events before interaction, require a bounded observation/network-idle contract, and bind the finalized capture to the exact target, route set, budget metrics, and Drupal result-state fingerprint. Capture is capped at 64 declared routes and one 120-second aggregate wall-clock budget. Matching requests, incomplete coverage, unsettled network, budget exhaustion, or unavailable Chrome fail closed. Packet-authored before-consent records remain non-authoritative diagnostics.
