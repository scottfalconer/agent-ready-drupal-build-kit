...
Independent claim evidence must be JSON with `schemaVersion: public-kit.independent-claim-evidence.2`. Each claim entry binds `claimId`, completion-area `gate`, canonical `gateId`, `targetBaseUrl`, and `checkedAt` to concrete checks containing `name`, `method`, `result: pass`, and `observation`. A single evidence manifest may cover multiple claims through a `claims` array; a generic status-only file cannot clear them.

Structured artifacts are fail-closed contracts. Every `gateId` must exist in `gates.json`; status, verdict, disposition, route-behavior, and review-state values must use their template enums. Passing summaries cannot contain failed, blocked, unknown, unevaluated, or open children, and summary counts must reconcile with their source rows. Accepted ownership, route, config, readback, and completion facts cannot contradict their children.

Each report `gateResults` row includes `evaluator`, `evaluatorRan`, `status`, and `errors`. Machine status can be `pass` only when that named evaluator ran and produced no finding. Human gates remain `human_review`, and out-of-scope machine gates remain `not_evaluated`; absence of an attributed error is never enough to manufacture a pass. Every new non-human gate must register a named evaluator in `MACHINE_GATE_EVALUATORS` and wire that evaluator into the applicable packet or live inspection before it can pass. A missing registry entry fails closed, even if a caller incorrectly lists the gate as evaluated.

The verifier also parses the key local-rebuild handoff records rather than trusting an appended sentence: operator identity and run evidence, one resolved installed-substrate and Recipe-fit decision, a dispositioned gap list, evidence-backed human decisions or an explicit none declaration, accepted off-road review, durable intent that is current (an empty list requires both empty live/readback architecture and no off-road decisions, or a named, evidenced `empty_justification`), and a named positive maintainer verdict. `launch-checklist.md` and `production-target.md` never mechanically authorize the narrower complete-local-rebuild claim, but each must carry run identity plus exactly one checked local-claim scope declaration; an untouched template copy fails packet readiness.
...
The default `verify.mjs` exits zero only when it authorizes completion, including human-recorded gate acceptance. It exits `2` when the packet and live target are structurally valid but completion is incomplete. The report verdict is `machine-incomplete` when any required non-human handoff gate or Drupal-runtime check has not passed; it is `mechanically-verified-awaiting-human-signoff` only when every required machine gate passed and human acceptance is the sole remaining blocker. It exits `1` when packet or live-target validation fails.
