...
- `G-CODE-01`: when custom modules or themes exist, bind the verifier-owned inventory fingerprint and every stable lexical/registration source-surface ID to one capability in `drupal-readback.json.implementationQuality.customCodeInventory`. Each capability uses a verifier-derived stable ID, stable `AC-` acceptance criteria, and the ordered core → installed Drupal CMS → Recipe → maintained contrib → custom-exception ladder with a decision, rationale, and packet-local evidence at every stage. The aggregate- and per-extension-bounded realpath-safe inventory covers direct custom extension directories, extension metadata, routes/controllers, hooks or callbacks, services, plugins, Twig, JavaScript and built assets, CSS, SDC definitions, shipped config, and tests; nested production extensions fail closed instead of being assigned to a parent. For enabled extensions, the runtime path must match the inventoried path; each representative live custom request declares expected anonymous access, runs through Drupal's parameter enhancers, and is checked through the access manager, while install config plus active optional/active config are independently validated with Drupal's schema checker. Inactive optional config and disabled custom extensions remain inventoried without a live typed-config or route claim. This gate does not run syntax, standards, static-analysis, cacheability/identity, or discovered-test checks, so it is only the bounded inventory/router/schema subset of the broader custom-code quality contract.
