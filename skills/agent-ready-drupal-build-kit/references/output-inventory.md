...
- `G-DRUPAL-01`: compare configured field widgets/formatters against Drupal plugin `field_types` plus class applicability, distinguish registered extra fields from stale typed components, inventory active tracked Canvas content templates against exact public and embedded display ownership, inventory YAML/callback/attribute custom routes through the live router, match and convert a declared representative Request before request-aware anonymous access, run verifier-owned custom-PHP quality checks or require named unsupported-tool exceptions, and live-prove Pathauto/editor alias behavior for node and non-node public detail owners.
