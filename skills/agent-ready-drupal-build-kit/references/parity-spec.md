...
- It re-fetches a seeded random sample of the full `routes` array on every run and records the seed in the report so the sample is reproducible. Every route row must declare a non-empty source path and target path; a blank `targetPath` is a packet failure and a live failure, never a silent skip out of the sample population. The report records the route-row and drift-classification counts so population shrinkage is visible.
- `menuAndFooterLinksChecked` and `renderedLinksChecked` must be per-link records that the verifier re-fetches. Same-origin links record `href`, observed `status`, and `finalPath`; links to other origins record the absolute `href` with `disposition: "external"` and are validated without being fetched. Bare text labels fail packet verification.
- It crawls same-origin rendered links from the front page and sampled content pages, including query-string variants of already-verified paths. A rendered link that returns 4xx — including an unrewritten link imported inside body content — is a real route-gate failure, not import noise. Rewrite imported links at import time; the only escape hatch is a per-link `owner_accepted_broken` disposition with `acceptedBy` and `rationale` on the matching link record, and it excuses exactly that path-plus-query, not every query variant. A crawled same-origin link that redirects off-origin (a vanity outbound link) is recorded with its destination and passes.
- When a route row maps a source path to a different target path, the mapping must materialize as a permanent HTTP 301 (or 308) on the target that lands on the mapped path, or the row must carry a `noRedirectDisposition` with `acceptedBy` and `rationale`. A documented mapping whose source path 404s on the target never passes silently. Mappings are keyed by the full source path including its query string — `/page.aspx?ID=1` and `/page.aspx?ID=2` are separate mappings that are each fetched — and two rows that map the same source path to different targets fail as a conflict instead of collapsing to one check.
