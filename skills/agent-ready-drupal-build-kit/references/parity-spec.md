...
The managed-browser source/target visual floor includes a separate bounded `computedStyleShadow` observation for semantic body, heading, primary navigation/action, form-control, and card/listing anchors at desktop and mobile. It records allowlisted computed style values and loaded font-family names without page text or URLs. Font replacement, known web-font load loss, extreme font-size ratios, and obvious control-token drift are diagnostic only; missing anchors remain `insufficient_evidence`. This shadow record has no completion authority and never changes the structural floor's status.
