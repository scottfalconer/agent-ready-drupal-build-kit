...
main() {
  ...
  if ! browser_runtime_override_matches "$override" "$rendered"; then
    browser_runtime_error "the managed verifier override is stale or changed. Run setup-browser-runtime.sh from the host before launching an agent."
    return 1
  fi
  ...
  printf 'Recreating only the %s service...\n' "$BROWSER_RUNTIME_SERVICE"
