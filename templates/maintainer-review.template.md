...
- Per-capability custom extension solution ladders and stable `capabilityId` bindings (core -> installed/disabled Drupal CMS -> current composer-installable supported Recipes -> maintained compatible contrib -> custom):
- Custom module last-resort rationale and named acceptance:
- Theme ownership finding dispositions:
- Canvas/component asset libraries under the active theme:
...
- [ ] Every public Canvas/component owner was rendered anonymously under the active public theme. `browser-evidence.json` binds route/theme/provider/component IDs to the declared library contract and browser-observed asset URLs, maps direct or equivalent assets, and includes passing computed-style checks for CSS, interaction checks for JavaScript, or a rationale plus visual check when no library is required. Live `system.theme:default`, fetched `<link>`/`<script>` URLs, and route-bound packet-local browser artifacts match those claims; registration, component markup, fake URLs, or free text are not asset evidence.
...
- [ ] Every distinct capability in each custom module and theme has its own complete structured solution ladder with a unique stable `capabilityId`: exact need and stable-ID acceptance criteria; Drupal core candidates; installed Drupal CMS capabilities including disabled submodules; current composer-installable Recipes with version, Drupal/core compatibility, support status, and security-advisory coverage; maintained contrib with version, Drupal/core compatibility, maintenance status, security-advisory coverage, and adoption evidence; and a named, dated custom decision with narrowest scope and revisit trigger. Empty stages carry discovery evidence, and every partial candidate references the exact unmet criterion IDs.
- [ ] Every runtime-relevant custom source file and discovered hook/function/class/individual registration/config-schema/Twig/JavaScript-behavior/stylesheet/SDC surface is live-reconciled by stable ID and current file hash, bound exactly once to a capability of the same extension, and given a specific responsibility. Excluded tests, fixtures, build scripts, vendor/tooling/generated files, minified bundles, source maps, and binary assets are not invented as runtime evidence. Broad capability grouping is reviewed for semantic cohesion rather than trusted because a scanner cannot infer it.
- [ ] Every custom route, discovered controller, and machine-discovered custom-theme Twig/PHP ownership finding references a `capabilityId` declared by its owning extension plus the exact implementing `sourceSurfaceIds`; routing registrations, class/file surfaces, and nearest procedural function surfaces cannot borrow another capability elsewhere in the same file. Controller inventory records include path and extension even when no route owns them. Theme findings cover literal internal paths/aliases, theme-PHP metadata injection, hand-written search forms, and unsuffixed `views-*.html.twig` base-template overrides outside test/fixture/tooling data. Accepted theme exceptions and false positives are reviewed; `replace_with_drupal_owner` findings remain blocking until remediation removes them from the runtime scan.
- [ ] Custom modules are named for reusable Drupal capabilities. Any exact source-host/site-name token match has a structured exception with a named owner, rationale, and packet-local evidence; substring and generic-token matches do not count.
...
- [ ] YAML, callback, and attribute custom routes are reconciled through the live Drupal router; each declares an HTTP method, matches and converts a real representative Request, and has effective allowed/denied anonymous access. Allowed routes bind to the public-only route matrix; denied routes bind to the exact structured private-route inventory.
