...
test('declared query route variants are fetched and state-bound distinctly while fragments are ignored', async () => {
  ...
  await withHttpServer(
    ...
    async (baseUrl) => {
      ...
      assert.deepEqual(
        first.routeChecks.filter((route) => route.targetPath.startsWith('/search')).map((route) => route.targetPath).sort(),
        ['/search?type=speaker&year=2026', '/search?year=2026&type=speaker']
      );
      assert.deepEqual(
        first.buildState.routeManifest.filter((route) => route.path.startsWith('/search')).map((route) => route.path).sort(),
        ['/search?type=speaker&year=2026', '/search?year=2026&type=speaker']
      );
